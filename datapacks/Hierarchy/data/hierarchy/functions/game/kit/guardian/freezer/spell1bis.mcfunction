execute as @e[tag=AffectByOrange] at @s run tag @e[type=armor_stand,scores={Class=34,Timer=0..},distance=..3] add Collision
execute as @e[type=armor_stand,scores={Class=34,Timer=..-140}] at @s run scoreboard players set @e[scores={EffRegen=..1020},tag=AffectByBlue,distance=..5] EffRegen 1020
execute as @e[type=armor_stand,scores={Class=34,Timer=..-140}] at @s run scoreboard players set @e[scores={EffSpeed=..1020},tag=AffectByBlue,distance=..5] EffSpeed 1020
execute as @e[type=armor_stand,scores={Class=34,Timer=-141}] at @s run scoreboard players set @e[scores={EffDamage=..2},tag=AffectByOrange,distance=..5] EffDamage 2
execute as @e[type=armor_stand,scores={Class=34,Timer=-141}] at @s run scoreboard players set @e[scores={EffSlow=..3040},tag=AffectByOrange,distance=..5] EffSlow 3040
execute as @e[type=armor_stand,scores={Class=34,Timer=-141}] at @s run scoreboard players operation @e[tag=AffectByOrange,distance=..5] TouchID = @s ID
